    ...
    <main className="grid flex-1 place-items-center bg-gradient-to-b from-blue-50 to-white px-4 py-10 sm:px-6 lg:px-8">
      <Card className="w-full max-w-xl rounded-[2rem] border-blue-100 shadow-xl shadow-blue-100/60">
        <CardHeader className="text-center">
          ...
          <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700"><UserPlus aria-hidden="true" /></div>
          <CardTitle className="text-3xl">Tạo tài khoản CHẠM</CardTitle>
          <p className="text-slate-600">Lưu từ yêu thích, tiến độ học và điểm quiz.</p>
        </CardHeader>
        <CardContent>
          <form className="grid gap-4" onSubmit={onSubmit}>
            <label className="grid gap-2"><span className="font-bold text-slate-800">Họ tên</span><Input value={fullName} onChange={(event) => setFullName(event.target.value)} required autoComplete="name" /></label>
            ...
            <label className="grid gap-2"><span className="font-bold text-slate-800">Nhập lại mật khẩu</span><Input type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} required autoComplete="new-password" /></label>
            ...
            <p className="text-center text-slate-600">Đã có tài khoản? <Link href="/dang-nhap" className="font-bold text-blue-700 hover:text-blue-900">Đăng nhập</Link></p>
          </form>
        </CardContent>
      </Card>
    </main>
